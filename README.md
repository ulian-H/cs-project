...
- **Line Notify 中繼支援（透過 GAS 解決 CORS、啟用後端排程）**
...
- **測試傳送通知**：即時傳送測試訊息到 Line（經由 GAS 中繼解決 CORS）
- **事件提醒**：設定事件提醒時，若有 Line Notify token，會嘗試在提醒時間傳送通知
- **草案建立通知**：當接受 AI 草案建立事件時，可自動傳送通知到 Line
